/**
 * Lists Recruitee departments alongside the current Sanity service category docs,
 * so an editor can fill in `serviceCategory.recruiteeId` and have the jobs sync
 ...
 * paired with the currently-mapped Sanity service category (if any).
 */
...
type CategoryRow = {_id: string; name: string; locale: string | null; recruiteeId: string | null}
...
async function main() {
  ...
  const [departments, offers, categories] = await Promise.all([
    fetchDepartments(recruiteeConfig),
    fetchAllOffers(recruiteeConfig),
    client.fetch<CategoryRow[]>(
      `*[_type == "serviceCategory"]{_id, "name": coalesce(name[_key == "en"][0].value, name[0].value, _id), locale, recruiteeId} | order(name asc)`,
    ),
  ])
  ...
  const categoryByRecruiteeId = new Map<string, CategoryRow[]>()
  for (const e of categories) {
    if (!e.recruiteeId) continue
    const list = categoryByRecruiteeId.get(e.recruiteeId) ?? []
    list.push(e)
    categoryByRecruiteeId.set(e.recruiteeId, list)
  }

  console.log('\n== Recruitee departments ==\n')
  for (const dept of departments) {
    const deptOffers = offersByDept.get(dept.id) ?? []
    const mapped = categoryByRecruiteeId.get(String(dept.id)) ?? []
    const mappedLabel =
      mapped.length > 0
        ? mapped.map((m) => `${m.name}${m.locale ? ` [${m.locale}]` : ''}`).join(', ')
        : '— (no Sanity service category mapped)'
  ...
  console.log('== Sanity service category docs ==\n')
  for (const e of categories) {
  ...
  const unmapped = departments.filter((d) => !categoryByRecruiteeId.has(String(d.id)))
  if (unmapped.length > 0) {
    console.log(`Heads up: ${unmapped.length} Recruitee department(s) have no Sanity service category mapped:`)
