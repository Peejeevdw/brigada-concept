...
// Echte client-logo's voor de "Clients"-wall (links-uitgelijnde SVG's, zoals op
// /brand). Verdubbeld zodat de cycling-wall genoeg logo's heeft om te roteren.
...
// v5 — full-screen intro-video (Bunny HLS) die eerst speelt; vlak voor het einde
// neemt de wordmark-hero het over.
const INTRO_VIDEO_SRC =
  "https://vz-9610a6cf-b06.b-cdn.net/a2e6db10-09fb-470d-b584-91882c46df8c/playlist.m3u8";
// Seconden vóór het video-einde waarop de wordmark-animatie start. Klein, zodat
// de video bijna volledig uitspeelt en de wordmark er net op het einde overheen komt.
const INTRO_LEAD_S = 0.8;
...
const ConceptV5 = ({
  ...
  // Intro-video flow: heroStarted = de wordmark-hero begint (vlak voor het
  // video-einde); introGone = de video-overlay is uitgefade en uit de DOM.
  const [heroStarted, setHeroStarted] = useState(false);
  const [introGone, setIntroGone] = useState(false);
  // Custom sound-cursor op de intro-video (start muted door autoplay-policy).
  const [introMuted, setIntroMuted] = useState(true);
  const introVideoRef = useRef<HTMLVideoElement>(null);
  const introProgressRef = useRef<HTMLDivElement>(null);
  ...
  // Intro-video (Bunny HLS): muted autoplay, speelt één keer. Vlak voor het einde
  // (INTRO_LEAD_S) — of bij ended/fout/timeout — start de wordmark-hero.
  useEffect(() => {
    ...
    const startHero = () => setHeroStarted(true);
    // Smooth progress + near-end trigger via rAF (timeupdate vuurt maar ~4×/s,
    // wat schokt). Stopt zodra de wordmark-hero het overneemt.
    let rafId = 0;
    const tick = () => {
      if (heroStartedRef.current) return;
      if (video.duration) {
        const pct = Math.min(100, (video.currentTime / video.duration) * 100);
        if (introProgressRef.current) introProgressRef.current.style.width = `${pct}%`;
        if (video.currentTime >= video.duration - INTRO_LEAD_S) startHero();
      }
    ...
    // Veiligheidsnet enkel voor het geval de video nooit start (laadfout): zodra
    // hij speelt wissen we dit, zodat de volledige video (28s) afspeelt en enkel
    // timeupdate/ended bepaalt wanneer de wordmark het overneemt.
    const fallback = window.setTimeout(startHero, 15000);
    const clearFallback = () => window.clearTimeout(fallback);
    video.addEventListener("playing", clearFallback, { once: true });

    let hls: Hls | null = null;
    // Prefer hls.js zodat we het start-level zelf kiezen. autoStartLoad:false +
    // een op-resolutie gekozen startLevel = scherpe start i.p.v. een lage rung
    // die opramp't (zie HlsBackgroundVideo / memory "HLS prefer hls.js / sharp start").
    if (Hls.isSupported()) {
      ...
      hls = instance;
      instance.loadSource(INTRO_VIDEO_SRC);
      instance.attachMedia(video);
      instance.on(Hls.Events.MANIFEST_PARSED, () => {
        const levels = instance.levels || [];
        if (levels.length) {
          // Hoogste resolutie, dan de kleinste die de gerenderde box (height × dpr)
          // nog dekt — scherp vanaf de eerste fragment.
          const highestIdx = levels.reduce(
            (best, l, idx) =>
              (l.height || 0) > (levels[best].height || 0) ? idx : best,
            0,
          );
          ...
          instance.startLevel = pick;
        }
        instance.startLoad();
        tryPlay();
      });
      instance.on(Hls.Events.ERROR, (_e, data) => {
        if (data?.fatal) startHero();
      });
    } else {
      video.src = INTRO_VIDEO_SRC;
      video.addEventListener("loadedmetadata", tryPlay, { once: true });
      video.addEventListener("error", startHero, { once: true });
    }
    video.addEventListener("ended", startHero);

    return () => {
      window.clearTimeout(fallback);
      cancelAnimationFrame(rafId);
      video.removeEventListener("playing", clearFallback);
      video.removeEventListener("ended", startHero);
      if (hls) {
        try {
          hls.destroy();
        } catch (_) {
  ...
  // Unmount de overlay nadat hij is uitgefade (na heroStarted).
  useEffect(() => {
    if (!heroStarted) return;
    const id = window.setTimeout(() => setIntroGone(true), 1200);
    return () => window.clearTimeout(id);
  }, [heroStarted]);

  // Scroll vergrendelen zolang de intro-video loopt; vrijgeven bij heroStarted.
  // Lenis stoppen (smooth-scroll luistert anders gewoon door, ondanks overflow)
  // + naar boven forceren, zodat skip altijd bovenaan de hero uitkomt.
  useEffect(() => {
    heroStartedRef.current = heroStarted;
    if (heroStarted) {
  ...
  // Hoeveel de cases-collectie omhoog onder de hero wordt getrokken (vh). Lager =
  // de cases komen lager op de pagina.
  ...
  useEffect(() => {
    // Wacht met de wordmark-entrance tot de intro-video bijna klaar is (heroStarted).
    if (!heroStarted) {
      setRevealed(false);
      t.set(0);
      return;
  ...
  useEffect(() => {
    ...
    // Tijdens de intro-video staat scroll vergrendeld.
    if (!heroStartedRef.current) lenis.stop();
  ...
  */
  ...
  return (
    <main className="relative bg-brigada-black">
      {/* Intro-video (Bunny HLS) — full-screen, muted autoplay, speelt één keer.
          Fadet weg zodra de wordmark-hero start (vlak voor het video-einde),
          waarna de rode hero eronder zichtbaar wordt. */}
      {!introGone && (
        <div
          aria-hidden
          className={`fixed inset-0 z-[120] bg-brigada-black transition-opacity duration-[1200ms] ease-in-out ${
            heroStarted ? "opacity-0" : "opacity-100"
          }`}
          style={{ pointerEvents: heroStarted ? "none" : "auto" }}
        >
          <video
            ref={introVideoRef}
            ...
            preload="auto"
          />
          {/* Custom sound-cursor (zelfde als op de cases) — klik = geluid aan/uit. */}
          <SoundCursorOverlay muted={introMuted} onClick={toggleIntroSound} />

          {/* Onderbalk — subtiele progress-balk (links, vult de breedte) + skip-knop
              (rechts), op dezelfde hoogte. */}
          <div className="absolute bottom-[clamp(16px,3vw,28px)] left-0 right-0 z-[2] flex items-center gap-4 px-[clamp(16px,3vw,28px)]">
            <div className="h-[2px] flex-1 bg-white/15">
              <div ref={introProgressRef} className="h-full w-0 bg-white/70" />
            </div>
            <button
              type="button"
              onClick={() => setHeroStarted(true)}
              aria-label="Intro overslaan"
              className="flex shrink-0 items-center gap-2 rounded-full border border-white/25 bg-black/30 px-3 py-2 text-[12px] font-medium uppercase tracking-[0.08em] text-white backdrop-blur-md transition-colors hover:bg-black/50"
            >
      ...
      <section ref={heroRef} className="relative z-10" style={{ height: `${heroVh}vh` }}>
        <div className="sticky top-0 h-screen select-none overflow-hidden bg-brigada-black">
          ...
          >
            {/* Background video — same full-bleed reel as the section above the
                footer (/bg-red.mp4). */}
            <video
      ...
      {/* Clients — "Proud not loud"-layout: sticky intro links, cycling logo
          wall rechts (3-up, zwarte logo's op wit via .logo-wall--ink). */}
      <section className="relative z-10 w-full bg-white text-brigada-black">
        <div className="grid w-full grid-cols-1 gap-x-12 gap-y-12 px-[clamp(24px,5vw,72px)] py-[clamp(140px,22vh,300px)] md:grid-cols-2 md:items-center">
          {/* Left — intro, verticaal gecentreerd t.o.v. de logo-wall */}
          ...
          {/* Right — cycling logo wall (3-up, zwarte logo's op wit). Zelfde
              logo-grootte als /brand via .logo-wall--brand; logo's zijn van
              nature brigada-black, dus geen ink-filter nodig. */}
          <Reveal>
            <div className="logo-wall--brand">
