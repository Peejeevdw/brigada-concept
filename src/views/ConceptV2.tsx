...
import BunnyReelPlayer from "@/components/BunnyReelPlayer";
  ...
  */
  ...
  return (
    <main className="relative bg-brigada-black">
      ...
      {/* /press/launch hero-video — onder de baseline-sectie. Zelfde HeroMedia-
          pipeline als die pagina; valt weg als er geen launch-video in Sanity
          staat. */}
      {pressHeroMedia && (
        <div className="relative z-10" style={{ marginTop: `-${videoPull}vh` }}>
          {pressHeroMedia.type === "video" && /\.m3u8(\?|#|$)/i.test(pressHeroMedia.src) ? (
            // Klik-om-af-te-spelen reel met glass play-knop (Osmo-look); geen autoplay.
            <BunnyReelPlayer
              src={pressHeroMedia.src}
              poster={pressHeroMedia.poster}
              className="h-[90vh] w-full overflow-hidden"
            />
          ) : (
            <HeroMedia
              media={pressHeroMedia}
