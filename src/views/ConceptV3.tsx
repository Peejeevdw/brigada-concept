...
import MiniShowreel from "@/components/MiniShowreel";
...
// Rode brio-achtergrond in de hero — tijdelijk uit; zwarte achtergrond i.p.v.
const SHOW_HERO_BRIO = false;
...
// Scène 4 — de vier domeinen, rechts, scroll-gedreven roller (zelfde single-slot
// look als de agencies, maar verspringt op scroll i.p.v. automatisch).
const DOMAINS = ["Brand", "Marketing", "People", "Product"];

// Reel-bron (Bunny HLS) — hardcoded voor de showreel in scène A.
const REEL_SRC =
  "https://vz-9610a6cf-b06.b-cdn.net/dd423d77-1d47-4934-85e5-d16c5f7a3213/playlist.m3u8";
...
const ConceptV3 = ({
  ...
  const [heroVh, setHeroVh] = useState(420);
  ...
  */
  ...
  // ---- "This is us" scroll-hero (v3) ----
  // "This is us" + de reel staan vast links; de agency-namen cyclen rechts via het
  // Osmo "Looping Words"-principe (elastic auto-loop, top/bottom fade, zonder
  // selector) — zie het LoopingWords-component.
  //
  // Scène-overgang op scroll (rode video blijft sticky): scène A (This is us +
  // reel + agencies) fadet uit, scène B ("We build a new and unique agency") in.
  // Verticale, gecentreerde scènes (zie A.1–A.5). Elke scène is een absolute
  // inset-0 laag met een p-gedreven opacity-venster; de rode video blijft sticky.
  // Scène A — This is us / reel / agency, verticaal gestapeld + gecentreerd.
  const sceneAOpacity = useTransform(p, [0, 0.1, 0.16], [1, 1, 0]);
  // Reel-klik uitschakelen zodra scène A wegfadet (niet op een onzichtbare reel klikken).
  const sceneAPointer = useTransform(p, (v) => (v < 0.13 ? "auto" : "none"));
  // Scène B — "We build a new and unique agency".
  const sceneBOpacity = useTransform(p, [0.14, 0.21, 0.28, 0.35], [0, 1, 1, 0]);
  const sceneBY = useTransform(p, [0.14, 0.21], ["24px", "0px"]);
  // Scène 4a — "Around four interconnected domains.".
  const scene4Opacity = useTransform(p, [0.33, 0.4, 0.47, 0.54], [0, 1, 1, 0]);
  const scene4Y = useTransform(p, [0.33, 0.4], ["24px", "0px"]);
  // Scène 4b — de domeinen, groot gecentreerd, scroll-gedreven roller.
  const domainsSceneOpacity = useTransform(p, [0.52, 0.59, 0.84, 0.9], [0, 1, 1, 0]);
  // Scène 5 — "With a new name that carries legacy and boosts energy".
  const scene5Opacity = useTransform(p, [0.88, 0.95], [0, 1]);
  const scene5Y = useTransform(p, [0.88, 0.95], ["24px", "0px"]);
  // Domeinen-roller positie (stapsgewijs op scroll): Brand → Marketing → People → Product.
  const DOM_STEP = 100 / DOMAINS.length; // % van de kolom = één regel
  const DOM_P0 = 0.6; // scroll waar Brand staat
  const DOM_P1 = 0.83; // scroll waar Product staat
  const DOM_SNAP = 0.28; // deel van elk slot dat de snelle overgang duurt; rest = hold
  const domStops: number[] = [];
  const domYs: string[] = [];
  {
    const N = DOMAINS.length;
    const slot = (DOM_P1 - DOM_P0) / N;
    for (let i = 0; i < N; i++) {
      const slotStart = DOM_P0 + i * slot;
      const snapEnd = slotStart + slot * DOM_SNAP;
      const prev = i === 0 ? 0 : -(i - 1) * DOM_STEP;
      domStops.push(slotStart, snapEnd);
      domYs.push(`${prev}%`, `${-i * DOM_STEP}%`);
    }
  }
  const domainsY = useTransform(p, domStops, domYs);
    ...
    <main className="relative bg-brigada-black">
      ...
      <section ref={heroRef} className="relative z-10" style={{ height: `${heroVh}vh` }}>
        <div className="sticky top-0 h-screen select-none overflow-hidden bg-brigada-black">
          {/* Sticky achtergrond — rode video (/bg-red.mp4). Zet SHOW_HERO_BRIO op
              true voor de brio-variant i.p.v. de video. */}
          {SHOW_HERO_BRIO ? (
            ...
            <video
              // iOS speelt alleen autoplay als de `muted` *property* gezet is —
              // imperatief zetten en play() kicken.
              ref={(el) => {
                if (!el) return;
                el.muted = true;
                el.play().catch(() => {});
          ...
          {/* Verticaal gecentreerde scènes (A.1–A.5). Elke scène = absolute
              inset-0 laag; de rode video blijft sticky eronder. */}

          {/* Scène A — This is us / reel / agency, verticaal en zo groot mogelijk:
              gespreid over de volledige hoogte (boven/midden/onder). */}
          <motion.div
            className="absolute inset-0 z-10 flex flex-col items-center justify-between px-[clamp(24px,5vw,72px)] pt-[clamp(92px,14vh,160px)] pb-[clamp(40px,8vh,104px)] text-center text-white"
            style={{ opacity: sceneAOpacity, pointerEvents: sceneAPointer }}
          >
            <motion.h1
              className="text-[clamp(44px,8vw,132px)] font-medium leading-[1.02] tracking-[-0.02em]"
            ...
            <motion.div
              className="h-[clamp(160px,36vh,440px)]"
              initial={{ opacity: 0, y: 20 }}
              animate={heroIn ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
            ...
            >
              <MiniShowreel
                src={REEL_SRC}
                poster={pressHeroMedia?.poster}
                className="aspect-[16/10] h-full"
              />
            </motion.div>

            <motion.div
              className="text-[clamp(44px,8vw,132px)] font-medium"
              initial={{ opacity: 0, y: 20 }}
              animate={heroIn ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
              transition={{ duration: 0.6, ease: EASE_OUT, delay: 0.6 }}
            >
              <LoopingWords words={AGENCIES} />
            </motion.div>
          </motion.div>

          {/* Scène B — "We build a new and unique agency" (gecentreerd). */}
          <motion.div
            className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center px-[clamp(24px,5vw,72px)] text-center text-white"
            style={{ opacity: sceneBOpacity, y: sceneBY }}
          >
            <h2 className="max-w-[16ch] text-[clamp(40px,5.5vw,88px)] font-medium leading-[1.05] tracking-[-0.02em]">
              We build a new and unique agency
            </h2>
          </motion.div>

          {/* Scène 4a — "Around four interconnected domains." (gecentreerd). */}
          <motion.div
            className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center px-[clamp(24px,5vw,72px)] text-center text-white"
            style={{ opacity: scene4Opacity, y: scene4Y }}
          >
            <h2 className="max-w-[14ch] text-[clamp(40px,5.5vw,88px)] font-medium leading-[1.05] tracking-[-0.02em]">
          ...
          {/* Scène 4b — de domeinen, groot gecentreerd, scroll-gedreven roller. */}
          <motion.div
            className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center px-[clamp(24px,5vw,72px)] text-white"
            style={{ opacity: domainsSceneOpacity }}
          >
            <div className="looping-words text-[clamp(44px,6.5vw,112px)] font-medium uppercase tracking-[-0.01em]">
              <div className="looping-words__viewport">
                <motion.div className="looping-words__list" style={{ y: domainsY }}>
                  {DOMAINS.map((d) => (
                    <div key={d} className="looping-words__p whitespace-nowrap">
                      {d}
                    </div>
                  ))}
                </motion.div>
          ...
          >
            <h2 className="max-w-[16ch] text-[clamp(40px,5.5vw,88px)] font-medium leading-[1.05] tracking-[-0.02em]">
              With a new name that carries legacy and boosts energy
            </h2>
          </motion.div>
        </div>
      </section>
