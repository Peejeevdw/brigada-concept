// k-means dominant color extraction. Returns colors sorted shadow -> highlight.
// Self-contained: works on any image or video URL via a hidden canvas.
...
export const extractDominantColors = async (
  ...
  const px = await sampleFrame(src, kind, 96);
  if (!px) return [];
  const pixels: number[][] = [];
