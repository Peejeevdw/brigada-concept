...
// Same curve as a callable easing function — useTransform's `ease` option needs a
// function, not a bezier array (the array form only works in animate/transition).
const EASE_OUT_FN = cubicBezier(0.16, 1, 0.3, 1);
...
const ConceptLab = () => {
  ...
  // Pre-roll lead-in (vh). The hero is lengthened by this; during the pre-roll
  // only the wordmark is visible, parked 40px above the viewport bottom over the
  // reel, and the whole group rises so the tagline comes up from below. 0 = off.
  const [introVh, setIntroVh] = useState(19);
  // Ease the pre-roll rise (expo-out) instead of linear — fixes the "heavy" feel.
  const [introEase, setIntroEase] = useState(false);
  // Tagline slide-in timing, as a % of the pre-roll: where SHARP starts (start)
  // and the gap between SHARP→BEATS→LOUD (stagger). Each word's slide spans ~20%.
  const [taglineStart, setTaglineStart] = useState(36);
  const [taglineStagger, setTaglineStagger] = useState(15);
  ...
  */
  ...
  // ---- Pre-roll: wordmark starts parked at the bottom, rises on scroll ----
  // The hero is lengthened by introVh; settleAt is the progress fraction the
  // pre-roll consumes. `q` is a remapped progress (0 at settle → 1 at pin-release)
  // that drives ALL the original choreography unchanged — just shifted to start
  // after the pre-roll. introVh = 0 reduces this exactly to the old behaviour.
  const pinRange = heroVh - 100;
  const settleAt = introVh / (introVh + pinRange);
  const q = useTransform(p, [settleAt, 1], [0, 1], { clamp: true });
  // Measured px offset that parks the wordmark's bottom 40px above the viewport
  // bottom; eases to 0 across the pre-roll. Applied to the whole content group,
  // so the tagline/paragraph sit below the fold and rise up with it.
  const introOffset = useMotionValue(0);
  const introProgress = useTransform(p, [0, settleAt], [1, 0], {
    clamp: true,
    ease: introEase ? EASE_OUT_FN : undefined,
  });
  const introY = useTransform(
    [introProgress, introOffset],
    ([prog, off]: number[]) => prog * off
  );
  useEffect(() => {
    const measure = () => {
      const h = logoBoxRef.current?.offsetHeight ?? 0;
      introOffset.set(Math.max(0, window.innerHeight / 2 - h / 2 - 40));
    };
    measure();
  ...
  // Runs to q=1 (the exact pin-release point) so the group never freezes before
  // the sticky lets go — the text keeps scrolling instead of pausing at the end.
  const groupY = useTransform(q, [0, 1], ["0vh", "-64vh"]);
  // Logo fades as it scrolls off the top.
  const logoOpacity = useTransform(q, [0.56, 0.78], [1, 0]);
  // Extra lead so the logo scrolls up a bit faster than the rest of the group.
  const logoLead = useTransform(q, [0, 1], ["0vh", "-26vh"]);
  // Per-line clip-up reveal, staggered.
  const line0Y = useTransform(q, [0.22, 0.5], ["115%", "0%"]);
  const line1Y = useTransform(q, [0.4, 0.68], ["115%", "0%"]);
  const lineYs = [line0Y, line1Y];

  // Tagline words slide up out of a clip mask during the PRE-ROLL (driven by the
  // raw progress p, before settle), sequentially: SHARP → BEATS → LOUD. Because
  // q is still 0 here, baselineSize stays at its smallest, so the words slide in
  // at the small size and only START growing once the main choreography (q) kicks
  // in afterwards — the slide-in and the growth never overlap.
  const wStart = taglineStart / 100;
  const wGap = taglineStagger / 100;
  const wSpan = 0.2; // how long each single word takes to slide in (% of pre-roll)
  const word0Y = useTransform(p, [settleAt * wStart, settleAt * (wStart + wSpan)], ["115%", "0%"], { clamp: true });
  const word1Y = useTransform(p, [settleAt * (wStart + wGap), settleAt * (wStart + wGap + wSpan)], ["115%", "0%"], { clamp: true });
  const word2Y = useTransform(p, [settleAt * (wStart + 2 * wGap), settleAt * (wStart + 2 * wGap + wSpan)], ["115%", "0%"], { clamp: true });
  const wordYs = [word0Y, word1Y, word2Y];
  ...
  return (
    <main className="relative bg-brigada-black">
      ...
      {/* Dev-only tuning panel — pre-roll + tagline slide-in. The earlier params
          (hero length / cut / video pull / baseline) are kept in state but hidden
      ...
      {import.meta.env.DEV && (
        ...
        >
          ...
          <label className="mb-2 block">
            Aanloop — {introVh}vh <span className="text-white/40">(↓ = sneller/lichter)</span>
            <input
              type="range"
              min={0}
              max={120}
              value={introVh}
              onChange={(e) => setIntroVh(+e.target.value)}
          ...
          <label className="mb-2 flex items-center gap-2">
            <input
              type="checkbox"
              checked={introEase}
              onChange={(e) => setIntroEase(e.target.checked)}
              className="accent-white"
            />
            Ease aanloop <span className="text-white/40">(expo-out vs lineair)</span>
          </label>
          <label className="mb-2 block">
            Tagline start — {taglineStart}% <span className="text-white/40">van aanloop</span>
            <input
              type="range"
              min={0}
              max={70}
              value={taglineStart}
              onChange={(e) => setTaglineStart(+e.target.value)}
              className="mt-1 w-full accent-white"
            />
          </label>
          <label className="block">
            Tagline stagger — {taglineStagger}% <span className="text-white/40">tussen woorden</span>
            <input
              type="range"
              min={0}
              max={30}
              value={taglineStagger}
              onChange={(e) => setTaglineStagger(+e.target.value)}
              className="mt-1 w-full accent-white"
            />
          </label>
        </div>
      ...
      <section ref={heroRef} className="relative z-10" style={{ height: `${heroVh + introVh}vh` }}>
        <div className="sticky top-0 h-screen select-none overflow-hidden bg-brigada-black">
          ...
          {/* Pre-roll wrapper — parks the whole group low at the start (only the
              wordmark visible, 40px above the viewport bottom) and eases it back
              to 0 as the pre-roll completes, so the tagline rises up from below. */}
          <motion.div className="pointer-events-none absolute inset-0 z-10" style={{ y: introY }}>
          ...
          <motion.div className="pointer-events-none absolute inset-0" style={{ y: groupY }}>
          ...
          >
            {TAGLINE.map((word, i) => (
              <span key={word} className="block overflow-hidden">
                <motion.span
                  className="block uppercase leading-[1.1] tracking-[-0.015em]"
                  style={{
                    fontFamily: SANS,
                    fontStretch: "125%",
